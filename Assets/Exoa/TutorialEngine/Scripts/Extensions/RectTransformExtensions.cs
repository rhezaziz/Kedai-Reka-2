...
public static class RectTransformExtensions
{
	...
	public static Rect GetRectFromOtherParent(this RectTransform obj, RectTransform other)
	{
		...
		Camera objCam = null;
		if (objCanvas.renderMode == RenderMode.ScreenSpaceCamera)
			objCam = objCanvas.worldCamera;
		else if (objCanvas.renderMode == RenderMode.ScreenSpaceOverlay)
			objCam = null;

		Vector2 topLeftScreenPos = RectTransformUtility.WorldToScreenPoint(objCam, topLeftCorner);
		Vector2 bottomRightScreenPos = RectTransformUtility.WorldToScreenPoint(objCam, bottomRightCorner);

		Vector2 localPointTopLeft, localPointBottomRight;
		RectTransformUtility.ScreenPointToLocalPointInRectangle(other, topLeftScreenPos, objCam, out localPointTopLeft);
		RectTransformUtility.ScreenPointToLocalPointInRectangle(other, bottomRightScreenPos, objCam, out localPointBottomRight);
