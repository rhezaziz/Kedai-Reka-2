...
public class Bahan_Mover : MonoBehaviour
{
    //***************************************************************************//
   ...
   // private GameObject grill;           //grill machine to process burgers
    ...
    void Start()
    {
        ...
        serverPlate = GameObject.FindGameObjectWithTag("Bowl");
...
*/
    ...
    void manageDiscard()
    {
        ...
        if (Physics.Raycast(ray, out hitInfo))
        {
            ...
            if (objectHit.tag == "overburnedIngredient" && objectHit.name == gameObject.name)
            {
                ...
                Bahan.itemIsInHand = true;  //we have an ingredient in hand. no room for other ingredients!
  ...
  /*
    ...
    */
    ...
    {
        if (!Bahan.itemIsInHand)    //if nothing is being dragged
            return;
        else
        {
...
*/
    ...
    IEnumerator processRawIngredient()
    {
        ...
        if (t >= 1)
        {
            ...
            if (v >= 1 && gameObject.tag != "deliveryQueueItem" && !Bahan.itemIsInHand)
            {
                isProcessing = false;
                isProcessed = true;
                isOverburned = true;
    ...
    */
    ...
    void checkCorrectPlacement()
    {
        ...
        if (distanceToPlate < minDeliveryDistance && tempBowl.check(needKatsu, this.gameObject))
        {
           ...
           /* transform.parent = serverPlate.transform;
            ...
            //we no longer need this ingredient's script (ProductMover class)
            //GetComponent<Bahan_Mover>().enabled = false;
            Destroy(gameObject);
   ...
   /*
    ...
    {
        while (Bahan.itemIsInHand || target == serverPlate)
        {
...
/*
    ...
    {
        while (Bahan.itemIsInHand || target == trashBin)
        {
