...
*/
   ...
   // public GameObject money3dText;              //3d text mesh over customers head after successful delivery
    ...
    void Awake()
    {
        ...
        deliveryPlate = GameObject.FindGameObjectWithTag("Bowl");
    ...
    void Init()
    {
        ...
        if (customerNeeds == 0)
        {
            ...
            {
                customerNeeds = Random.Range(0, availableProducts.Length - 1);
            }
        }
...
*/
        ...
        productImage = Instantiate(availableProducts[customerNeeds]) as GameObject;
        productImage.name = "customerNeeds";
        productImage.transform.parent = requestBubble.transform;
        productImage.transform.localPosition = Vector3.zero;
        ...
        for (int j = 0; j < productIngredients; j++)
        {
            ...
            helperIngredients[j].GetComponent<Bahan_Mover>().enabled = false;
    ...
    IEnumerator goToSeat()
    {
        ...
        while (!isOnSeat)
        {
            ...
            if (transform.position.x >= destination.x)
            {
                ...
                requestBubble.transform.parent.gameObject.SetActive(true);
...
*/
    ...
    {
        bool temp = false;
        if(order.Length == ReceiveOrder.Count)
        {
            for(int i = 0; i <  ReceiveOrder.Count; i++)
            {
                for (int x = 0; x < order.Length; x++)
                {
                    temp = false;
                    if (ReceiveOrder[i] == order[x])
                    {
                        temp = true;
                        break;
                    }
                    //Debug.Log($"{order[x]} == {ReceiveOrder[i]} (_{temp}_)");
                }
            }
            return temp;
        }
