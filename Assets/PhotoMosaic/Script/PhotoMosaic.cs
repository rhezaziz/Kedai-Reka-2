...
namespace sunny{
	public class PhotoMosaic : MonoBehaviour {
		...
		private class SortItem{
			public Color color;
			public PhotoMosaicDataSet.TextureData data;
		...
		public delegate void SpawnEventHandler(GameObject piece, PhotoMosaicDataSet.TextureData data);
		...
		public IEnumerator doGenerate(Texture2D target, PhotoMosaicDataSet dataSet){
			...
			/*
			uvRect.xMin = .5f-minSize/(float)target.width/2;
			uvRect.xMax = .5f+minSize/(float)target.width/2;
			uvRect.yMin = .5f-minSize/(float)target.height/2;
			uvRect.yMax = .5f+minSize/(float)target.height/2;
			*/
			...
			foreach(PhotoMosaicDataSet.TextureData texData in dataSet.textures){
